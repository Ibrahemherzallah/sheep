...
const SheepDetails = () => {
  ...
  const [deleteTrahModal, setDeleteTrahModal] = useState(false)
  const { id } = useParams();
  ...
  useEffect(() => {
    const fetchSheep = async () => {
      try {
        const response = await fetch(`https://thesheep.top/api/sheep/${id}`);
  ...
  const handleDeleteTrah = async () => {
    try {
      const response = await fetch(`https://thesheep.top/api/trah/delete/${selectedPregnancy}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`
        },
      ...
      toast({
        title: "تم حذف الطراح بنجاح",
        description: "تم إزالة السجل من قاعدة البيانات"
      });
      ...
      toast({
        title: "خطأ",
        description: err.message || "فشل حذف الطراح",
        variant: "destructive"
      });
    }
  };
...
// Usage
  ...
  return (
    <div className="p-6 space-y-6 animate-fade-in">
        ...
        <TabsContent value="births" className="space-y-4" dir={'rtl'}>
          {sheep.sex === 'male' ? (
            ...
            <>
              ...
              {sheep?.trahCases?.length > 0 && (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                        <Baby size={18} />
                        <span>حالات الطراح</span>
                      </CardTitle>
                      <CardDescription>تاريخ لجميع حالات الطراح لهذه النعجة</CardDescription>
                    </CardHeader>
                    <CardContent>
                          <Table>
                            <TableHeader>
                            ...
                            <TableBody>
                              {sheep?.trahCases.map((record) => (
                                  <TableRow key={record.id}>
                                    {
                                      record.trahDate
                                          ?<TableCell>{new Date(record.trahDate).toISOString().split('T')[0]}</TableCell>
                                          : 'غير متوفر'
                                    }
                                    <TableCell>
                                      <div>
                                    ...
                                    <TableCell>
                                      <Button
                                          onClick={() => {
                                            setDeleteTrahModal(true);
                                            setSelectedPregnancy(record._id);
                                          }}
                                          variant="outline"
                                          className="ml-2"
                                      >
                                        <Trash size={18} />
                                        <span>حذف الطراح</span>
                                      </Button>
                                    </TableCell>
                                  </TableRow>
                              ))}
      ...
      <Dialog open={deleteTrahModal} onOpenChange={setDeleteTrahModal}>
        <DialogContent dir="rtl">
          <DialogHeader>
            <DialogTitle>تأكيد حذف الطراح</DialogTitle>
            <DialogDescription>
              هل أنت متأكد أنك تريد حذف هذا السجل؟ هذا الإجراء لا يمكن التراجع عنه.
            </DialogDescription>
          </DialogHeader>

          <DialogFooter>
            <Button variant="outline" onClick={() => setEditPregnancyModal(false)}>
              إلغاء
            </Button>

            <Button variant="destructive" onClick={handleDeleteTrah}>
              حذف
            </Button>
          </DialogFooter>
        </DialogContent>
